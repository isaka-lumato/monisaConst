(function ($) {
  ...
  let banner__slider = new Swiper(".banner__slider", {
    ...
    autoplay: {
      delay: 6000, // Slightly longer delay for better UX
    ...
    on: {
      init: () => {
        // Animate project cards on initial load with single trigger
        setTimeout(() => {
          const activeSlide = document.querySelector(
            ".banner__slider .swiper-slide-active"
          );
          if (activeSlide) {
            const projectCards = activeSlide.querySelectorAll(".project-card");
            projectCards.forEach((card, index) => {
      ...
      slideChange: () => {
        ...
        const timer = document.querySelector(".timer-swiperAfter");
        if (timer) {
      ...
      slideChangeTransitionEnd: () => {
        // Trigger project card animations after slide transition is complete
        setTimeout(() => {
          const activeSlide = document.querySelector(
            ".banner__slider .swiper-slide-active"
          );
        ...
        }, 100); // Reduced delay to prevent double animation
      },
      slideChangeTransitionStart: () => {
        // Remove animations from all cards during transition
